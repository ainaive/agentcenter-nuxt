...
- **Demo accounts can sign in** — `bun run db:seed` now plants Better-Auth credential rows alongside the six demo users on `*@agentcenter.dev`. Password comes from `SEED_PASSWORD`, which is now required — the seed fails fast if it isn't set so a misconfigured environment can't hand out a predictable default. Run as `SEED_PASSWORD=anything-you-like bun run db:seed`, then click around the approval flow as amy (super-admin) or dao (company admin for `cloud`) without signing up a fresh account. (#43)
...
- **演示账号可登录** —— `bun run db:seed` 现在会为 6 个 `*@agentcenter.dev` 演示用户写入 Better-Auth 凭据行。密码必须通过 `SEED_PASSWORD` 显式提供；未设置时种子脚本会立即失败，避免误配置环境泄露可预测的默认凭据。运行示例：`SEED_PASSWORD=anything-you-like bun run db:seed`，随后即可以 amy（超级管理员）或 dao（`cloud` 公司级管理员）等身份体验完整审核流程。 (#43)
